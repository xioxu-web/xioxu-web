...
package io.microservice.oauth2.cloud.gateway.filter;
...
import io.microservice.oauth2.cloud.gateway.config.GatewayConfigProperties;
...
import io.microservice.oauth2.common.core.exception.ValidateCodeException;
...
public class ValidateCodeGatewayFilter extends AbstractGatewayFilterFactory<Object> {
	...
	public GatewayFilter apply(Object config) {
		return (exchange, chain) -> {
			...
			boolean isIgnoreClient = configProperties.getIgnoreClients().contains(WebUtils.getClientId( (org.springframework.http.server.ServerHttpRequest) request ));
			try {
				...
				return response.writeWith(Mono.create(monoSink -> {
					try {
						byte[] bytes = objectMapper.writeValueAsBytes(ResultMsg.failed(errMsg));
