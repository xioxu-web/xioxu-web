 ...
 */
@Slf4j
public class OauthSentinelInvocationHandler implements InvocationHandler {
    ...
    OauthSentinelInvocationHandler(Target<?> target, Map<Method, InvocationHandlerFactory.MethodHandler> dispatch,
            ...
            if (methodMetadata == null) {
                ...
                try {
                    ...
                    if (fallbackFactory != null) {
                        ...
                        // 若是R类型 执行自动降级返回R
                        if (ResultMsg.class == method.getReturnType()) {
                            log.error("feign 服务间调用异常", ex);
                            return ResultMsg.failed(ex.getLocalizedMessage());
                        }
                        else {
                            throw ex;
                        }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof SentinelInvocationHandler) {
            OauthSentinelInvocationHandler other = (OauthSentinelInvocationHandler) obj;
