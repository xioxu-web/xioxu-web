package com.example.mybatis_plus_demo.generator;
...
//        实现 ITypeConvert 接口自定义数据库 字段类型 转换为自己需要的 java 类型，内置转换类型无法满足可实现 IColumnType 接口自定义
        ...
        pc.setEntity("domain");//Java 实体类模板
        pc.setMapper("mapper");//mapper 模板
        pc.setParent("com.example.mybatis_plus_demo");//父包名。如果为空，将下面子包名必须写全部， 否则就只需写子包名
        ...
        strategy.setInclude(scanner("userinfo").split(","));//需要包含的表名，允许正则表达式（与exclude二选一配置）
